...
def push_server_chan(sendkey, title, content):
    ...
    try:
        ...
        if res.get('code') == 0:
            ...
            log(f"❌ 微信推送失败: {res.get('message')}")
    except Exception as e:
        log(f"❌ 微信请求异常: {e}")

def push_dingtalk(webhook, secret, results_objs):
    if not webhook: 
        log("⚠️ 未设置 DINGTALK_WEBHOOK，跳过推送")
        return
    ...
    push_server_chan(sc_sendkey, f"GLaDOS {greeting}简报", md_text)

def main():
    log("🚀 GLaDOS 乔布斯禅意版启动...")
    raw_cookie = os.environ.get("GLADOS_COOKIE", "")
    if not raw_cookie:
        log("❌ 未配置 GLADOS_COOKIE 环境变量")
        return
